...
export function Footer() {
  ...
  useEffect(() => {
    fetch("/api/studio-info")
      .then((res) => res.json())
      .then((data: StudioInfo) => setInfo(data))
          ...
          <div>
            ...
            <ul className="space-y-4 mb-6">
              <li className="flex items-start gap-3">
                ...
                <span className="text-gray-400 text-sm">{info?.address ?? ""}</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="w-5 h-5 text-neonPink flex-shrink-0" />
                <span className="text-gray-400 text-sm">{info?.phone ?? ""}</span>
              </li>
              <li className="flex items-center gap-3">
            ...
            <a
              ...
              className="block relative overflow-hidden rounded-lg border border-white/10 hover:border-neonGreen transition-all duration-300 group"
            >
              <div className="relative h-48 w-full bg-gradient-to-br from-neonGreen/10 to-neonPink/10">
