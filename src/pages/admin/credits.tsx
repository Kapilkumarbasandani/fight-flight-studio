...
import { CreditCard, Plus, Minus, Search, TrendingUp, TrendingDown, X, Check, RefreshCcw } from "lucide-react";
...
export default function AdminCredits() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Keep selectedMember in sync with latest data
        setSelectedMember(prev => {
          if (!prev) return null;
          const updated = data.find((m: Member) => m.id === prev.id);
          return updated ?? prev;
        });
      } else {
  ...
  const handleSubmitAdjustment = async () => {
    ...
    try {
      ...
      // Refresh members list and history from DB
      await fetchMembers();
      await fetchHistory();
            ...
            <div className="space-y-3">
              {adjustmentHistory.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400">No manual credit adjustments yet</p>
                  <p className="text-gray-500 text-sm">Adjustments made here will appear in this history</p>
                </div>
              ) : (
