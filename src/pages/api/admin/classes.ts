...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Hard delete - fully remove from DB so it no longer appears anywhere
      ...
      // Get all active classes for admin (inactive/deleted ones are excluded)
      const classes = await classesCollection
        .find({ active: { $ne: false } })
        .sort({ day: 1, time: 1 })
        .toArray();

      // Attach real enrolled count from bookings for each class
      const bookingsCollection = db.collection('bookings');
      const classesWithEnrolled = await Promise.all(
        classes.map(async (cls) => {
          const enrolled = await bookingsCollection.countDocuments({
      ...
      return res.status(200).json(classesWithEnrolled);
    } else {
