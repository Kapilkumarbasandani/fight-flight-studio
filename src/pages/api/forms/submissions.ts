...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      ...
      // Return all forms as pending for dev test users (non-ObjectId IDs)
      if (!ObjectId.isValid(userId)) {
        const allForms = [
          { id: 'waiver',    title: 'Member Waiver',              description: 'Required liability waiver for all members',        required: true,  fields: [] },
          { id: 'health',    title: 'Health Questionnaire',       description: 'Medical history and fitness assessment',           required: true,  fields: [] },
          { id: 'emergency', title: 'Emergency Contact Form',     description: 'Contact information for emergencies',              required: true,  fields: [] },
          { id: 'media',     title: 'Photo/Video Release',        description: 'Media consent for marketing materials',            required: false, fields: [] },
          { id: 'training',  title: 'Private Training Interest',  description: 'Request one-on-one coaching sessions',            required: false, fields: [] },
        ];
        return res.status(200).json({
          forms: allForms.map(f => ({
            ...f,
            status: 'pending',
            date: f.required ? 'Not yet completed' : 'Optional form',
            submissionId: null,
          })),
