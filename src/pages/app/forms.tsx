...
import { FileText, CheckCircle, Clock, Download, ExternalLink, Upload, AlertCircle, X as XIcon } from "lucide-react";
...
export default function FormsPage() {
  ...
  const handleFormClick = (form: any) => {
    if (form.status === "completed") {
      ...
      setSignature(''); // Reset signature when opening form
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    ctx.lineTo(x, y);
    ctx.strokeStyle = '#10b981'; // neonGreen color
    ctx.lineWidth = 2;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.stroke();
  ...
  const handleFormSubmit = async () => {
    ...
    const hasSignatureField = selectedForm?.fields?.includes('Signature');
    ...
    try {
      ...
      const response = await fetch('/api/forms/submissions', {
        ...
        body: JSON.stringify({
          ...
          signature: signature || '' // Use actual signature data
        })
      });
                      ...
                      <div className="space-y-2">
                        <div className="relative">
                          <canvas
                            ...
                            className="w-full h-48 bg-white/5 border-2 border-dashed border-white/20 rounded-lg cursor-crosshair hover:border-neonGreen/50 transition-all"
                            style={{ touchAction: 'none' }}
                          />
                          {!signature && (
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              <span className="text-white/40 text-sm">Draw your signature here</span>
                            </div>
                          )}
                        </div>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-red-500/10 text-red-400 rounded-lg text-sm font-bold hover:bg-red-500/20 transition-all duration-300"
                        >
